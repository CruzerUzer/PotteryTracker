        ...
        stmt.finalize((finalizeErr) => {
          ...
          db.close((closeErr) => {
            if (closeErr) {
              ...
              const seedProcess = spawn('node', [join(__dirname, 'seed-data.js')], {
              ...
              seedProcess.on('close', (code) => {
                if (code === 0) {
                  ...
                  console.log('Note: Seed data import skipped or no seed data available.');
                }
                process.exit(0);
              });
              
              seedProcess.on('error', (err) => {
                console.log('Note: Could not run seed data import:', err.message);
                process.exit(0);
              });
            }
          });
